...
|-----------|-------------|
| **Perceivable** | Present information and UI components in ways users can perceive |
| **Operable** | Make UI components and navigation operable |
| **Understandable** | Make information and UI operations understandable |
| **Robust** | Make content interpretable by a variety of user agents, including assistive technologies |
...
| 2.5.7 Dragging Movements | Provide alternative operations for drag | When implementing future D&D features |
...
- **European Accessibility Act (EAA)**: Effective June 28, 2025; mandatory when providing services in the EU
...
[React Aria](https://react-spectrum.adobe.com/react-aria/) is an accessible UI component library developed by Adobe. It provides over 50 components and hooks with the following built-in capabilities.
...
- **Style Freedom**: Can be combined with any styling solution such as TailwindCSS
...
| `Select` | Select boxes | Custom Select |
...
React Aria components expose state through data attributes. They can be styled directly with Tailwind.
...
1. **Button**: Replace `<button>` → `<Button>`, `onClick` → `onPress`
2. **Input**: Replace `<input>` → `<TextField>` + `<Input>` to integrate label and error
3. **Select**: Replace `<select>` → `<Select>` + `<ListBox>` for improved keyboard operation
4. **Modal**: Replace custom implementation → `<Modal>` + `<Dialog>` for automatic focus trapping
...
| `focus-visible` styles | ✅ Partial | Implemented for Button, Input, Select |
...
|---------|----------|--------|
| Modal accessibility | High | Focus trapping, role, ESC key |
| Color contrast | High | Improve coral color |
| `aria-describedby` | Medium | Error message association |
| `aria-required` | Medium | Indicate required fields |
| Skip link | Medium | Jump to main content |
...
Modals must satisfy the following requirements.
...
#### Design That Does Not Rely Solely on Color

Combine visual cues other than color when indicating state changes.
...
// Error message (immediate notification)
...
| Tool | Usage | Detection Range |
|------|-------|-----------------|
| axe-core | Automated checking in CI/CD | ~40% |
...
2. Do buttons respond to `Enter`/`Space`?
...
- Is image alternative text appropriate?
...
- [ ] Closes on ESC key
- [ ] Focus returns to the original element after closing
...
- [ ] Automated testing with axe-core executed
- [ ] All features operable with keyboard only
- [ ] Page read-aloud verified with VoiceOver/NVDA
- [ ] No layout breakage at 200% zoom
- [ ] Animations verified with reduced-motion setting
