...
This document summarizes the tool selection and implementation policy for API integration testing in this project.

**Note**: This project uses `@hono/zod-openapi` and automatically generates an **OpenAPI 3.1 specification** at the `/doc` endpoint. This provides high compatibility with OpenAPI-based testing tools.

## Related Documents

- `docs/testing/api-testing.md` - Latest API test implementation policy (minimal mocking + table-driven)
...
| **API test client** | Hono testClient | Type-safe, Hono-native, IDE autocomplete support |
...
Since this project exposes an OpenAPI spec at `/doc`, the following tools can be leveraged.
...
- Adopted by Spotify, JetBrains, Red Hat, etc.
...
- Tests become insufficient if the spec is incomplete
...
**Generates a mock server** from the OpenAPI spec and operates as a validation proxy.
...
A lightweight framework that defines test scenarios in **YAML format**.
...
- CLI-based test automation

**Best suited for**:
- Cases where manual API testing is primary with partial automation
...
| **Memory usage** | ~30% less | Issues at scale |
...
- **Use real DB**: Uses the MySQL instance started by `pnpm dev` as-is
...
export default defineConfig({
  test: {
    ...
    // Tests run sequentially (to avoid DB state conflicts)
    pool: 'forks',
    poolOptions: {
      forks: {
        singleFork: true,
...
│   │   ├── createTestContainer.ts # Container with mocked externals
 ...
 * - Repository: Real (actual DB connection)
...
export const createTestContainer = (): Container => {
  ...
  // Actual Repositories (real DB connection)
 ...
 * - Inject real Container (only externals mocked)
 * - Bypass authentication and inject test user
 */
...
/**
 * Execute a test within a transaction (auto-rollback)
 */
...
describe('Item API', () => {
  ...
  describe('PUT /me', () => {
    it('Can update item information', async () => {
      ...
      // Verify it is reflected in the DB
...
describe('Order API', () => {
  ...
  describe('POST /tasks/:id/complete', () => {
    it('Generates results on order processing (external services mocked)', async () => {
      ...
      // Results are generated (response from mocks)
...
describe('Item API with Transaction Rollback', () => {
  it('Test item processing (auto-rollback)', async () => {
    ...
    // Rollback complete at this point
  })
})
```
    ...
    items:
      ...
      - name: Run API tests
        ...
        env:
          DATABASE_URL: mysql://root:root@localhost:3306/my_app_test
          NODE_ENV: test
          # No API keys needed for external services (they are mocked)
...
|------|--------|-------------|-----------|
| **Unit** | Domain, UseCase | Mocks | Per PR |
...
describe('UserUseCase', () => {
  ...
  it('Returns Err for duplicate email', async () => {
...
| **Integration** | Vitest + testClient | Integration tests for API routes (DI mocking) |
